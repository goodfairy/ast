...
class ViewsJsonQuery extends QueryPluginBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, ConfigFactoryInterface $config, CacheBackendInterface $cache, TimeInterface $time) {
  ...
  public function fetchFile($uri) {
    ...
    if (empty($parsed['host'])) {
      $module_path = DRUPAL_ROOT . '/' . \Drupal::service('module_handler')
        ->getModule('views_json_source')->getPath();
      if (!file_exists($module_path . $uri)) {
        throw new \Exception($this->t('Local file not found.'));
      }
      return file_get_contents($module_path . $uri);
    }
  ...
  public function execute(ViewExecutable $view) {
    ...
    $data = new \stdClass();
    try {
      $data->contents = $this->fetchFile($this->options['json_file']);
    }
    catch (\Exception $e) {
