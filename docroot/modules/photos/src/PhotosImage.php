...
class PhotosImage {
  ...
  public function delete($filepath = NULL, $count = FALSE) {
    ...
    if (!$filepath) {
      if ($count) {
        ...
        $db = \Drupal::database();
        $filepath = $db->query('SELECT uri FROM {file_managed} WHERE fid = :fid', [
          ':fid' => $fid,
        ])->fetchField();
      }
    }
    if ($filepath) {
      ...
      if (\Drupal::config('photos.settings')->get('photos_access_photos')) {
        ...
        if ($file_scheme == 'private') {
          ...
          $file_uris = $file_system->scanDirectory('private://photos/tmp_images', '~\b' . $basename . '\b~');
  ...
  public function pager($id, $type = 'pid') {
    ...
    if ($type == 'pid') {
      ...
      $db = \Drupal::database();
      $album_data = $db->query('SELECT data FROM {photos_album} WHERE pid = :pid', [
        ':pid' => $id,
      ])->fetchField();
    ...
    if ($previous_image_fid) {
      ...
      // Previous image.
      $t['prev_url'] = Url::fromUri('base:photos/image/' . $previous_image_fid)->toString();
    }
